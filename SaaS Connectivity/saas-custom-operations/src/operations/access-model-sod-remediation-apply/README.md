...
| `requestId` | Yes | Persist identity prefix. Bundled workflow sends `access-model-sod-remediation-apply`. Persist writes `{requestId}:{formInstanceId}`. In-flight dedupe still keys on `formInstanceId` for this command |

## Output

Persisted on the **apply persist identity** `{requestId}:{formInstanceId}` and returned on successful invoke:
...
Before catalog PATCH, the handler checks the result-source account at `{requestId}:{formInstanceId}` for a prior terminal apply (`applied` or `skipped-already-applied`). If that account is missing, it falls back to `access-model-sod-remediation-apply:{formInstanceId}` when that identity differs, then to the **legacy apply persist identity** (bare `{formInstanceId}`) as read-only input. When any lookup is a terminal apply, it skips the form definition lookup, tenant form instance list, and PATCH and returns `skipped-already-applied`. Replay persist always writes `{requestId}:{formInstanceId}`; the legacy account is never updated or deleted. Concurrent invokes for the same `formInstanceId` dedupe in-flight via a framework key that includes `formInstanceId` (not `requestId`).
...
| Invoke | `custom:access-model-sod-remediation-apply` with `formInstanceId: {{$.trigger.formInstanceId}}`, `formName: Access Model SOD Remediation`, and `requestId: access-model-sod-remediation-apply` |
| Persist key | Result-source **apply persist identity** `{requestId}:{formInstanceId}` → `access-model-sod-remediation-apply:{{formInstanceId}}` |
...
2. Read `access-model-sod-remediation-apply:status` from the invoke response, or Get Accounts on native identity `{requestId}:{formInstanceId}` (bundled: `access-model-sod-remediation-apply:{formInstanceId}`).
