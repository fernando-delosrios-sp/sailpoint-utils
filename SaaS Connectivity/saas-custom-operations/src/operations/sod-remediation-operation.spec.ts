...
const ensureFormDefinition = vi.fn()
const createRemediationInstance = vi.fn()
...
vi.mock('../isc/experimental-client', async (importOriginal) => {
    const actual = await importOriginal<typeof import('../isc/experimental-client')>()
    return {
        ...actual,
...
vi.mock('../isc/sod-form-service', async (importOriginal) => {
    const actual = await importOriginal<typeof import('../isc/sod-form-service')>()
    return {
        ...actual,
        ensureFormDefinition: (...args: unknown[]) => ensureFormDefinition(...args),
...
describe('sodRemediationOperation', () => {
    beforeEach(() => {
        ...
        ensureFormDefinition.mockResolvedValue('def-1')
        createRemediationInstance.mockResolvedValue('https://tenant.identitynow.com/form/instance-1')
    })
    ...
    it('returns formUrl and situationSummary on happy path', async () => {
        ...
        expect(ensureFormDefinition).toHaveBeenCalled()
        expect(createRemediationInstance).toHaveBeenCalledWith(
    ...
    it('creates form definition from seed when missing', async () => {
        ensureFormDefinition.mockResolvedValue('def-created')
        ...
        expect(ensureFormDefinition).toHaveBeenCalledWith(expect.anything(), 'New SOD Form', 'token-owner-id')
    })
    ...
    it('uses offline fallback owner for form definition create', async () => {
        ...
        try {
            ...
            expect(ensureFormDefinition).toHaveBeenCalledWith(
                expect.anything(),
                'SOD Remediation',
                'offline-owner'
            )
