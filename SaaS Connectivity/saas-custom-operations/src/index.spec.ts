...
describe('connector unit tests', () => {
    ...
    it('registers custom commands', async () => {
        ...
        expect(conn.handlers.has('custom:access-request-status')).toBe(true)
        expect(conn.handlers.has('custom:govgroup-emails')).toBe(true)
        expect(conn.handlers.has('custom:access-request-threshold')).toBe(true)
        expect(conn.handlers.has('custom:check-sod-pending')).toBe(true)
    })
})
