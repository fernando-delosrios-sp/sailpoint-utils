...
#### Scenario: Optional UI origin on line renderers

- **GIVEN** callers pass `uiOrigin` from `resolveUiOrigin(apiUrl)`
- **WHEN** flat access path or entitlement tree HTML is rendered
- **THEN** the library SHALL link entity display names when ids are available
- **AND** SHALL omit links when `uiOrigin` is undefined
