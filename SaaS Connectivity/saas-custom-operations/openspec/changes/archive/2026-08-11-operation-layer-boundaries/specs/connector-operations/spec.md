...
**Reason**: SOD-specific operation requirements belong in the per-operation capability `connector-operations/sod-remediation`, not the root connector-operations registry spec.

**Migration**: All SOD remediation scenarios are re-specified under `openspec/specs/connector-operations/sod-remediation/spec.md`. Root `connector-operations` retains only registration and layout rules.
