...
- **Unified SoD form HTML styling** — Adds shared builders under `src/lib/sod-form-html/` (type tags, icon suffixes, emoji legend, outcome panels). Both `custom:sod-remediation` and `custom:access-sod-remediation` pre-render six group column HTML `formInput` fields (`groupAContentsHtml`, `groupAContentsHtmlAsKept`, `groupAContentsHtmlAsRemoved`, and B equivalents) with seed `formConditions` that swap plain vs green/red outcome panels when the recipient selects `remediationSide`. sod-remediation uses icon-only line markers and a single legend in `situationSummaryHtml`; access-sod-remediation keeps nested AP trees without emojis. **Form definition migration:** adopt a **new** `formName` to pick up the updated seed layout; existing definition names are not patched.
...
- **`custom:access-sod-remediation`** — Scans enabled roles and access profiles in scope for intrinsic SoD policy violations (via `policyQuery` intersection, not predict), creates policy-owner remediation forms per (access item, policy) pair, and persists parent rollup on `requestId` plus per-form child accounts at `{requestId}:{accessItemId}:{policyId}`. Adds `src/isc/sod-policies/` and offline invoke via `payloads/access-sod-remediation-offline.json`.
...
- **`form-email-recipient` → `form-email-recipients`** — On `custom:sod-remediation` and `custom:access-sod-remediation`, the persist output key is renamed to `form-email-recipients` and typed as `string[]` (account schema `isMulti: true`). Values are single-element arrays wrapping the resolved owner email until multi-recipient resolution is added. Downstream workflows must update Get Accounts / Send Email JSONPath from `form-email-recipient` to `form-email-recipients`. Bundled `workflows/SOD Remediation - Violation Response.json` is updated.
