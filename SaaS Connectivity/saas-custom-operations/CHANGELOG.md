...
- **Access-model SoD request-scoped form dedupe** — `custom:access-model-sod-remediation` skips pending (ASSIGNED) form creation only when an instance already exists for the same parent `requestId`, access item, and policy (`formInput.parentRequestId`). New scans with a different `requestId` may launch forms even when an older pending instance exists from a prior run. Legacy instances without `parentRequestId` do not participate in dedupe.
- **Access-model SoD scan performance** — `custom:access-model-sod-remediation` loads assigned form instances once per scan, memoizes policy-owner email resolution, and caches access-item entitlement expansion within the scan loop to reduce ISC API volume on large catalogs.
...
- **Unified SoD form HTML styling** — Adds shared builders under `src/lib/sod-form-html/` (type tags, icon suffixes, emoji legend, outcome panels). Both `custom:sod-remediation` and `custom:access-model-sod-remediation` pre-render six group column HTML `formInput` fields (`groupAContentsHtml`, `groupAContentsHtmlAsKept`, `groupAContentsHtmlAsRemoved`, and B equivalents) with seed `formConditions` that swap plain vs green/red outcome panels when the recipient selects `remediationSide`. sod-remediation uses icon-only line markers and a single legend in `situationSummaryHtml`; access-model-sod-remediation keeps nested AP trees without emojis. **Form definition migration:** adopt a **new** `formName` to pick up the updated seed layout; existing definition names are not patched.
...
- **`custom:access-model-sod-remediation`** — Scans enabled roles and access profiles in scope for intrinsic SoD policy violations (via `policyQuery` intersection, not predict), creates policy-owner remediation forms per (access item, policy) pair, returns scan rollup counters on the invoke response, and persists per-form child accounts at `{requestId}:{accessItemId}:{policyId}`. Adds `src/isc/sod-policies/` and offline invoke via `payloads/access-model-sod-remediation-offline.json`.
