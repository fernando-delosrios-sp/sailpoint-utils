...
- **`custom:access-model-sod-remediation`** — Scans enabled roles and access profiles in scope for intrinsic SoD policy violations (via `policyQuery` intersection, not predict), creates policy-owner remediation forms per (access item, policy) pair, and persists parent rollup on `requestId` plus per-form child accounts at `{requestId}:{accessItemId}:{policyId}`. Adds `src/isc/sod-policies/` and offline invoke via `payloads/access-model-sod-remediation-offline.json`.
