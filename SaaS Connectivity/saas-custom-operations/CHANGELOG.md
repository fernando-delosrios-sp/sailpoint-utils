...
- **ABB operations on persisted-only output** — `custom:access-expiration-reminders` and `custom:check-sod-pending` declare never-persisted rollup fields on `response` and emit them via `ctx.respond` (envelope `summary`). Result-source account schema for expiration reminders no longer includes scan counters. Workflows that read those counters from a flat invoke body must use `summary.<field>`.
...
- **`custom:access-expiration-reminders`** — Discovers ACCESS_PROFILE assignments whose `removeDate` is exactly `expirationDays` UTC calendar days away (default `1`), creates capped manager reminder forms (max 25), and persists one notice account per form at `{requestId}:{identityId}:{accessProfileId}` with form URL and `form-email-*` fields for Send Email workflows. Returns reminder scan summary counters under the operation response envelope `summary` via `ctx.respond` (no parent account on bare `requestId`). Adds `src/isc/identities/` sunset search helpers, offline payload `payloads/access-expiration-reminders-offline.json`, and importable workflows `Access Expiration Reminders - Analysis` (daily 00:00 UTC, stable `requestId`) and `Access Expiration Reminders - Notification` (`idn:account-created` filtered by `operationName`). Does not apply the manager-selected `newExpirationDate` to ISC.
