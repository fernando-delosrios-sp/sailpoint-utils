###############################################################################################################################
# ConnectorBeforeModify rule that creates missing Active Directory OUs (and optional groups) before an account move.
#
...
#
# Upload this script as a Connector Rule (type: ConnectorBeforeModify) using the
...
#   - OUCreationEnabled (boolean): When true, create missing OUs from AC_NewParent. Default false skips the rule.
...
$ConnectorRuleName = "ConnectorBeforeModify - Create Active Directory OU"
...
###############################################################################################################################
# CUSTOM PROCESS CODE - Active Directory OU management (Before Modify)
###############################################################################################################################

$ctx.Session.Phase = "process"

try {
    Write-RuleLog -Level INFO -Message "Starting Active Directory OU management for account modification."
    Invoke-OrganizationalUnitManagement -TargetDistinguishedName (Get-RequestAttribute "AC_NewParent")
}
catch {
