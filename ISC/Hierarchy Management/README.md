...
IdentityAttribute rule for SailPoint ISC that builds a consolidated hierarchy path for an identity by traversing entitlement parent relationships stored in the description field.
...
- `Rule - IdentityAttribute - Hierarchy Management.xml` — IdentityAttribute rule that computes the hierarchy path.
...
1. Copy the rule XML into a new IdentityAttribute rule in your ISC tenant.
