...
| TCP 636 still not listening after EnableLdaps | AD DS needs the cert in **NTDS\My** with a **private key** (CertOpenStore), SAN/CN equal to `GetComputerNameEx(DnsFullyQualified)`, and a trusted chain (self-signed must be in LocalMachine\Root). Re-run EnableLdaps (it now fails if 636 is down), prefer **Create self-signed**, allow NTDS restart. Verify with `certutil -store -service NTDS My` and `Get-NetTCPConnection -LocalPort 636 -State Listen`. Reboot the DC if still silent. |
