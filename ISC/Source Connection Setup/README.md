...
| TCP 636 open but LDAPS clients fail / fall back to 389 | lsass may listen while presenting a wrong NTDS\My cert (e.g. RabbitMQ/FAM or short-name). EnableLdaps now removes competing NTDS\My certs and verifies a TLS handshake to the DC FQDN. Re-run and choose **Create self-signed**; confirm with `certutil -store -service NTDS My` that only the SailPoint LDAPS cert remains. |
