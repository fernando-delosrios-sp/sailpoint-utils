...
8. `git status --porcelain` empty on the **verification ref**. A dirty tree of this change's own work is resolved by **committing** it. Discarding the diff to turn the gate green is a defect, not a pass.
