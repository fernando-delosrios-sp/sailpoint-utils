provider "aws" {
  region = "eu-west-3" # DO NOT Change
}
    ...
    Statement = [
      {
        Effect   = "Allow"
        Action   = ["sns:Publish"]
        Resource = "arn:aws:sns:eu-west-3:937217723901:EntroSubsSns${var.sns_topic_arn_suffix}"
      }
    ]
  })
}
...
# Send IAM Role ARN to existing SNS Topic in eu-west-3
...
resource "aws_lambda_function" "sns_publish_lambda_function" {
  ...
  environment {
    variables = {
      TOPIC_ARN = "arn:aws:sns:eu-west-3:937217723901:EntroSubsSns${var.sns_topic_arn_suffix}"
    }
  }
}
