...
description: Present user gates as structured interactive choices at forks, confirmations, permission pauses, and requests for a value only the user holds. Run install to wire User gates into AGENTS.md or CLAUDE.md. Triggers include "install" and "install structured-choices".
---
...
At each **_gate_** — a fork, confirmation, permission pause, or value you need before you proceed — the choices reach the user as buttons they click, through the host's question tool.
...
- **Value requests** — a value only the user holds (an account id, an org domain, a display name), one gate per value

A value request is a gate even when you can name no candidate at all. Options carry whatever you can offer — what the live session reports, a documented default, your best guess — and the host's typed-answer escape hatch takes anything else. Where a value is the answer, "ask the user" means make the call.

**Skip the gate** when only one sensible path exists, the answer is free prose no candidate could approximate (a commit message, a paragraph of copy), or no user is available to answer (a CI or scheduled run) — pick a documented default and state the assumption in one line.
...
Read that as **option data for a call**: it describes the fork, or the value wanted, rather than drafting your message. Take one option per entry, an `id` from the entry's key or a slug of its label, and fold each entry's explanation into its `label`. A wording like "so they can accept it in a word" describes the click, not a request for prose.
...
- **Two or more fixed options** — a real fork, carrying the alternatives the user would actually weigh. A value request is the exception: it may carry a single candidate, because the typed answer is the point.
...
**Candidate count** governs identifying a target the repo or session can see: 1 match → inline one-line confirm; 2+ matches → full gate. A value request goes out as a call at any candidate count.
